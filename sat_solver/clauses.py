...
def _convert_to_int(formulas: List[Formula]) \
        -> Tuple[List[Set[int]],
                 Dict[Leaf, int]]:
    pass


def _create_translation(clauses: Collection[Formula])\
        -> Dict[str, int]:
    _OFFSET = 2  # Offset to avoid adding 0 and 1 to the translation table.
    translation: Dict[str, int] = {}
    unique_propositions = _get_leaves(clauses)
    for index, value in enumerate(unique_propositions):
        translation[value] = index + _OFFSET
    return translation
...
def _is_leaf(element: Leaf) -> bool:
    if isinstance(element, Proposition):
        return True
    elif isinstance(element.children[1], Proposition) \
